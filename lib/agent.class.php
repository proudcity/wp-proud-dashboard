...
class Agent extends ProudDashboard {
  ...
  function __construct() {
    ...
    add_action( 'wp_ajax_proud_dashboard_v1_trigger', array($this, 'ping') );
    add_action( 'wp_ajax_nopriv_proud_dashboard_v1_trigger', array($this, 'ping') );
  ...
  /**
   * Generic callback for ?action=proud_dashboard_v1_trigger&key&endpoint&siteId
   * Examples:
   * ?action=proud_dashboard_v1_trigger&key=plugins&endpoint=plugins&siteId=xxx
   * ?action=proud_dashboard_v1_trigger&key=accounts&endpoint=accounts&siteId=xxx
   * ?action=proud_dashboard_v1_trigger&key=stack&endpoint=stack/phpinfo&siteId=xxx
   */
  public function ping() {
    // print_r($_POST);

    $options = get_option( 'proud_dashboard_options' );
    // @todo: check that request is coming from plugin.proud_dashboard.com, or is properly nonced (for manual refreshes)
  ...
  // Callback for ?action=proud_dashboard_v1_trigger&key=plugins
  ...
  // Callback for ?action=proud_dashboard_v1_trigger&key=accounts
  private function accounts() {
    ...
    foreach ($users as $key => $user) {
      ...
      array_push( $out, array(
        ...
        'lastLogin' => strtotime( get_user_meta( $user->ID, 'proud_dashboard_last_login', true ) ),
      ) );
    }
  ...
  // Callback for ?action=proud_dashboard_v1_trigger&key=stack
  ...
  // Callback for ?action=proud_dashboard_v1_trigger&key=changeMode
  ...
  function last_login_save($user_login, $user) {
    ...
    update_user_meta( $user->ID, 'proud_dashboard_last_login', date('c') );
  }
